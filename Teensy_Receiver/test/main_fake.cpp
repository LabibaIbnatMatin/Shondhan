...
Stepper stepperB(10, 11);
Stepper stepperG(2, 3);
...
void gripper(int x)
{
    ...
    if (dir != 0)
    {
        if (dir != last_dir || !stepperG.isMoving)
        {
            stepperG.rotateAsync(16000 * dir);
            last_dir = dir;
        }
    }
    else
...
void base_arm()
{
    // Direction logic based on observation of original code:
    // val > 1550 -> b -= mp -> Negative move
    // val < 1450 -> b += mp -> Positive move

    int val = chVal[9];
    int dir = 0;

    if (val > 1550 && val <= 2000)
    {
        if (stepperB.getPosition() > thrs_base_b)
        {
            dir = -1;
        }
    }
    else if (val < 1450 && val >= 1000)
    {
        if (stepperB.getPosition() < thrs_base_f)
        {
            dir = 1;
        }
    }

    static int last_b_dir = 0;

    if (dir != 0)
    {
        if (dir != last_b_dir || !stepperB.isMoving)
        {
            stepperB.rotateAsync(900 * dir);
            last_b_dir = dir;
        }
    }
    else
...
setup()
{
    ...
    // Initialize TeensyStep4 timer system
    TS4::begin();
    ...
    if (ethernet_link_available)
{
    ...
    if (packetSize)
    {
        ...
        if (packetBuffer[0] == '#' && packetBuffer[2] == '#')
        {
            ...
            return;   // IMPORTANT → stop further parsing
        }
